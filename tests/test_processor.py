...
TEST_CASES = [
    ...
    (
        ...
        "O usuário está muito satisfeito.",
    ),
    ...
    (
        ...
        '{"sentiment": "neutral"},,',
        "neutral",
        "Dados de tradução ausentes ou inválidos.",
        "Baixa",
        [],
        "Falha na análise detalhada do LLM.",
    ),
    ...
    (
        ...
        # Adicionamos os campos faltantes para que a validação passe após a
        ...
        # O validador do modelo deve corrigir a intensidade para "Média" e o
        # resto deve ser mantido.
        "positive",
        "Muito bom!",
        "Média",
        ["geral"],
    ...
    (
        ...
        (
            '{"translation_pt": "Rápido e bonito", "sentiment": "positive", '
            '"intensity": "Alta", "aspects": "desempenho, design", '
            '"explanation": "Satisfeito com a velocidade e aparência."}'
        ),
        "positive",
        "Rápido e bonito",
        "Alta",
        ["desempenho", "design"],
        "Satisfeito com a velocidade e aparência.",
    ),
]

@pytest.mark.parametrize(
    (
        "test_name, review_input, llm_response, expected_sentiment, "
        "expected_translation, expected_intensity, expected_aspects, "
        "expected_explanation"
    ),
    TEST_CASES,
...
def test_map_llm_response_to_processed(
    ...
    assert isinstance(result, ReviewProcessed), (
        f"[{test_name}] O resultado não é uma instância de ReviewProcessed."
    )

    # Valida que os dados originais são sempre preservados
    assert result.user == review_input.user, (
        f"[{test_name}] Falha na preservação do usuário."
    )
    assert result.original == review_input.text, (
        f"[{test_name}] Falha na preservação do texto original."
    )
    assert result.language == review_input.language, (
        f"[{test_name}] Falha na preservação do idioma."
    )

    # Valida os campos processados
    assert result.sentiment == expected_sentiment, (
        f"[{test_name}] Falha na validação do sentimento."
    )
    assert result.translation_pt == expected_translation, (
        f"[{test_name}] Falha na validação da tradução."
    )
    assert result.intensity == expected_intensity, (
        f"[{test_name}] Falha na validação da intensidade."
    )
    assert result.aspects == expected_aspects, (
        f"[{test_name}] Falha na validação dos aspectos."
    )
    assert result.explanation == expected_explanation, (
        f"[{test_name}] Falha na validação da explicação."
    )
...
def test_analyze_reviews_logic():
    ...
    assert counts == expected_counts, (
        f"A contagem de sentimentos falhou. Esperado: {expected_counts}, "
        f"Obtido: {counts}"
    )
